/**
 * Lease & Real Estate Document Review — Use Case App (self-contained, no ES modules).
 * Supports file upload (TXT/PDF/DOCX) and direct text paste.
 * Sends lease text to the Nutanix AI endpoint via the local proxy for
...
(function () {
  ...
  /* =====================================================================
   * Inference — lease analysis via chat completions
   * =================================================================== */

  var SYSTEM_PROMPT =
    'You are a lease and real estate document review AI. Analyze lease text. Respond with JSON: ' +
    'riskScore (1-100), riskLevel (Low/Medium/High), summary, ' +
    'leaseTerms (array of {term, value, standard, deviation, risk (Low/Medium/High)}), ' +
    'obligations (array of {party, obligation, deadline, penalty}), ' +
    'deviations (array of {clause, severity (Low/Medium/High), description, recommendation}), ' +
    'keyDates (array of {event, date, description}). Respond ONLY with valid JSON.';
  ...
  function reviewLease(text, filename) {
    ...
    var userContent = 'Analyze the following lease or real estate document:\n\n' + trimmed;
    ...
    return chatCompletion(
      ...
      .then(function (response) {
        ...
        if (!parsed.riskScore) throw new Error('Response missing required fields');
        if (!parsed.leaseTerms) parsed.leaseTerms = [];
        if (!parsed.deviations) parsed.deviations = [];
        return parsed;
      });
  }
  ...
  var leaseTermsTbody = document.getElementById('lease-terms-tbody');
  var deviationsList = document.getElementById('deviations-list');
  ...
  function renderLeaseTerms(terms) {
    leaseTermsTbody.innerHTML = '';
    (terms || []).forEach(function (t) {
      var tr = document.createElement('tr');
      tr.innerHTML =
        '<td>' + esc(t.term) + '</td><td>' + esc(t.value) + '</td>' +
        '<td>' + esc(t.standard) + '</td><td>' + esc(t.deviation) + '</td>' +
        '<td><span class="risk-badge ' + riskClass(t.risk) + '">' + esc(t.risk) + '</span></td>';
      leaseTermsTbody.appendChild(tr);
    });
  }
  ...
  function renderDeviations(devs) {
    ...
    (devs || []).forEach(function (d) {
      ...
      deviationsList.appendChild(card);
    });
  }
  ...
  function displayAnalysis(result) {
    ...
    renderLeaseTerms(result.leaseTerms);
    renderDeviations(result.deviations);
    showResults();
  ...
  function renderHistoryInto(container, history) {
    ...
    if (!history || history.length === 0) {
      ...
      empty.textContent = 'No reviews yet. Upload or paste a lease to get started.';
  ...
  function runAnalysis(text, name) {
    ...
    setStage(stageSend, 'active', 'Sending lease to AI\u2026');

    reviewLease(text, name)
