/**
 * Contract Review & Analysis — Use Case App (self-contained, no ES modules).
...
(function () {
  ...
  /* =====================================================================
   * Inference — contract analysis via chat completions
   * =================================================================== */

  var SYSTEM_PROMPT =
    'You are a legal contract analysis AI. When given contract text, respond with a JSON object containing:\n' +
    ...
    'Analyze every material clause present. Respond ONLY with valid JSON, no markdown fences.';
  ...
  function analyzeContract(text, filename) {
    ...
    var userContent = 'Analyze the following contract:\n\n' + trimmed;
    ...
    return chatCompletion(
      ...
      .then(function (response) {
        ...
        if (!parsed.riskScore || !parsed.clauses) throw new Error('Response missing required fields');
        return parsed;
      });
  }
  ...
  function renderClauses(clauses) {
    ...
    (clauses || []).forEach(function (c) {
      ...
      tr.innerHTML =
        ...
        '<td><span class="risk-badge ' + riskClass(c.risk) + '">' + esc(c.risk) + '</span></td>' +
        '<td>' + esc(c.deviation) + '</td><td>' + esc(c.fallback) + '</td>';
