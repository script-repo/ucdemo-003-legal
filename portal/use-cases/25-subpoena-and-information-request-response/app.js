/**
 * Subpoena & Information Request Response — Use Case App (self-contained, no ES modules).
 * Supports file upload (TXT/PDF/DOCX) and direct text paste.
 * Sends subpoena text to the Nutanix AI endpoint via the local proxy for
...
(function () {
  ...
  /* =====================================================================
   * Inference — subpoena analysis via chat completions
   * =================================================================== */

  var SYSTEM_PROMPT =
    'You are a subpoena analysis AI. Analyze subpoenas and information requests. Respond with JSON: ' +
    'urgencyScore (1-100), urgencyLevel (Low/Medium/High/Critical), summary, ' +
    'obligations (array of {item, scope, deadline, burden (Low/Medium/High), recommendation}), ' +
    'deadlines (array of {event, date, description, extensionPossible (boolean)}), ' +
    'responseStrategy (string), ' +
    'potentialObjections (array of {basis, description, likelihood (Strong/Moderate/Weak)}), ' +
    'privilegeConsiderations (array of strings). Respond ONLY with valid JSON.';
  ...
  function analyzeSubpoena(text, filename) {
    ...
    var userContent = 'Analyze the following subpoena or information request:\n\n' + trimmed;
    ...
    return chatCompletion(
      ...
      .then(function (response) {
        ...
        if (!parsed.urgencyScore && parsed.urgencyScore !== 0) throw new Error('Response missing required fields');
        return parsed;
      });
  }
  ...
  function saveAnalysisToDb(result, name) {
    var title = name || 'Untitled Subpoena';
    return fetch(DB_API, {
      method: 'POST',
  ...
  var urgencyLevelEl = document.getElementById('urgency-level');
  var summaryEl = document.getElementById('summary-text');
  var obligationsTbody = document.getElementById('obligations-tbody');
  var deadlinesList = document.getElementById('deadlines-list');
  var responseStrategyEl = document.getElementById('response-strategy');
  var objectionsList = document.getElementById('objections-list');
  var privilegeList = document.getElementById('privilege-list');
  ...
  function burdenClass(burden) {
    var b = (burden || '').toLowerCase();
    if (b === 'high') return 'risk-high';
    if (b === 'medium') return 'risk-medium';
    return 'risk-low';
  }

  function likelihoodClass(lik) {
    var l = (lik || '').toLowerCase();
    if (l === 'strong') return 'risk-high';
    if (l === 'moderate') return 'risk-medium';
    return 'risk-low';
  }

  function renderUrgencyScore(score, level) {
    var displayEl = urgencyScoreEl.parentNode;
    if (displayEl) displayEl.className = 'risk-score-display ' + urgencyClass(level);
    urgencyScoreEl.textContent = score;
    urgencyScoreEl.className = 'risk-score-number ' + urgencyClass(level);
    urgencyLevelEl.textContent = level || '--';
    urgencyLevelEl.className = 'risk-badge ' + urgencyClass(level);
  }

  function renderSummary(text) { if (summaryEl) summaryEl.textContent = text || ''; }

  function renderObligations(obligations) {
    if (!obligationsTbody) return;
    obligationsTbody.innerHTML = '';
    (obligations || []).forEach(function (o) {
      var tr = document.createElement('tr');
      tr.innerHTML =
        '<td>' + esc(o.item || o.name || '') + '</td>' +
        '<td>' + esc(o.scope || '') + '</td>' +
        '<td>' + esc(o.deadline || '') + '</td>' +
        '<td><span class="risk-badge ' + burdenClass(o.burden) + '">' + esc(o.burden || '') + '</span></td>' +
  ...
  function renderDeadlines(deadlines) {
    ...
    (deadlines || []).forEach(function (d) {
      ...
      var ext = d.extensionPossible ? ' (extension possible)' : '';
      card.innerHTML =
        '<div class="deviation-header"><strong>' + esc(d.event || '') + '</strong>' +
        '<span>' + esc(d.date || '') + '</span></div>' +
        '<p class="deviation-desc">' + esc(d.description || '') + ext + '</p>';
      deadlinesList.appendChild(card);
    });
  }

  function renderResponseStrategy(text) {
    if (responseStrategyEl) responseStrategyEl.textContent = text || '';
  }

  function renderObjections(objections) {
    if (!objectionsList) return;
    objectionsList.innerHTML = '';
    (objections || []).forEach(function (o) {
      var card = document.createElement('div');
      var lik = (o.likelihood || '').toLowerCase();
      var sev = lik === 'strong' ? 'high' : (lik === 'moderate' ? 'medium' : 'low');
      card.className = 'deviation-item severity-' + sev;
      card.setAttribute('role', 'listitem');
      card.innerHTML =
        '<div class="deviation-header"><strong>' + esc(o.basis || '') + '</strong>' +
        '<span class="risk-badge ' + likelihoodClass(o.likelihood) + '">' + esc(o.likelihood || '') + '</span></div>' +
        '<p class="deviation-desc">' + esc(o.description || '') + '</p>';
  ...
  function renderPrivilege(items) {
    ...
    (items || []).forEach(function (p) {
      ...
      card.className = 'deviation-item severity-low';
      card.setAttribute('role', 'listitem');
      card.innerHTML = '<p class="deviation-desc">' + esc(typeof p === 'string' ? p : (p.text || p.description || '')) + '</p>';
      privilegeList.appendChild(card);
    });
  }

  function displayAnalysis(result) {
    renderUrgencyScore(result.urgencyScore, result.urgencyLevel);
    renderSummary(result.summary);
    renderObligations(result.obligations);
    renderDeadlines(result.deadlines);
    renderResponseStrategy(result.responseStrategy);
    renderObjections(result.potentialObjections);
    renderPrivilege(result.privilegeConsiderations);
    showResults();
  ...
  function renderHistoryInto(container, history) {
    ...
    if (!history || history.length === 0) {
      ...
      empty.textContent = 'No analyses yet. Upload or paste a subpoena to get started.';
    ...
    history.forEach(function (h) {
      ...
      var score = h.data ? (h.data.urgencyScore || 0) : 0;
      ...
      item.innerHTML =
        ...
        '<div class="history-meta">' +
          ...
          (score ? '<span class="risk-badge ' + urgencyClass(level) + '">' + score + '</span>' : '') +
        '</div>';
  ...
  function runAnalysis(text, name) {
    ...
    setStage(stageSend, 'active', 'Sending subpoena to AI\u2026');

    analyzeSubpoena(text, name)
  ...
  btnAnalyze.addEventListener('click', function () {
    ...
    if (file) {
      ...
      showLoading();
      setStage(stageExtract, 'done');
      runAnalysis(text, 'Pasted Subpoena');
    } else {
