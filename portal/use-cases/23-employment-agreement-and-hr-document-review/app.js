/**
 * Employment Agreement Review — Use Case App (self-contained, no ES modules).
 * Supports file upload (TXT/PDF/DOCX) and direct text paste.
 * Sends employment document text to the Nutanix AI endpoint via the local proxy for
...
(function () {
  ...
  /* =====================================================================
   * Inference — employment agreement analysis via chat completions
   * =================================================================== */

  var SYSTEM_PROMPT =
    'You are an employment agreement review AI. Analyze employment documents. Respond with JSON: ' +
    'riskScore (1-100), riskLevel (Low/Medium/High), summary, ' +
    'keyTerms (array of {term, value, standard, risk (Low/Medium/High), note}), ' +
    'restrictiveCovenants (array of {type (non-compete/non-solicit/confidentiality/IP-assignment), scope, duration, enforceability (Likely Enforceable/Possibly Unenforceable/Likely Unenforceable), recommendation}), ' +
    'issues (array of {clause, severity (Low/Medium/High), description, recommendation}), ' +
    'jurisdictionNotes (string). Respond ONLY with valid JSON.';
  ...
  function reviewEmploymentAgreement(text, filename) {
    ...
    var userContent = 'Analyze the following employment agreement or HR document:\n\n' + trimmed;
  ...
  function saveAnalysisToDb(result, name) {
    var title = name || 'Untitled Agreement';
    return fetch(DB_API, {
      method: 'POST',
  ...
  var termsTbody = document.getElementById('terms-tbody');
  var covenantsList = document.getElementById('covenants-list');
  var issuesList = document.getElementById('issues-list');
  var jurisdictionNotes = document.getElementById('jurisdiction-notes');
  ...
  function renderKeyTerms(terms) {
    ...
    (terms || []).forEach(function (t) {
      ...
      tr.innerHTML =
        '<td>' + esc(t.term) + '</td><td>' + esc(t.value) + '</td>' +
        '<td>' + esc(t.standard) + '</td>' +
  ...
  function renderRestrictiveCovenants(covenants) {
    covenantsList.innerHTML = '';
    (covenants || []).forEach(function (c) {
      var card = document.createElement('div');
      card.className = 'deviation-item';
      card.setAttribute('role', 'listitem');
      card.innerHTML =
        '<div class="deviation-header"><strong>' + esc(c.type) + '</strong>' +
        '<span class="risk-badge">' + esc(c.enforceability || '') + '</span></div>' +
        '<p class="deviation-desc"><strong>Scope:</strong> ' + esc(c.scope) + '</p>' +
        '<p class="deviation-desc"><strong>Duration:</strong> ' + esc(c.duration) + '</p>' +
        '<p class="deviation-rec"><strong>Recommendation:</strong> ' + esc(c.recommendation) + '</p>';
      covenantsList.appendChild(card);
    });
  }
  ...
  function renderIssues(issues) {
    ...
    (issues || []).forEach(function (d) {
      var card = document.createElement('div');
      card.className = 'deviation-item severity-' + (d.severity || 'low').toLowerCase();
      card.setAttribute('role', 'listitem');
      card.innerHTML =
        '<div class="deviation-header"><strong>' + esc(d.clause) + '</strong>' +
        '<span class="risk-badge ' + riskClass(d.severity) + '">' + esc(d.severity) + '</span></div>' +
        '<p class="deviation-desc">' + esc(d.description) + '</p>' +
        '<p class="deviation-rec"><strong>Recommendation:</strong> ' + esc(d.recommendation) + '</p>';
      issuesList.appendChild(card);
    });
  }

  function renderJurisdictionNotes(text) {
    jurisdictionNotes.textContent = text || 'No jurisdiction-specific notes identified.';
  }
  ...
  function displayAnalysis(result) {
    ...
    renderRestrictiveCovenants(result.restrictiveCovenants);
    renderIssues(result.issues);
    renderJurisdictionNotes(result.jurisdictionNotes);
    showResults();
  ...
  function renderHistoryInto(container, history) {
    ...
    if (!history || history.length === 0) {
      ...
      empty.textContent = 'No reviews yet. Upload or paste an agreement to get started.';
  ...
  function runAnalysis(text, name) {
    ...
    reviewEmploymentAgreement(text, name)
  ...
  btnAnalyze.addEventListener('click', function () {
    ...
    if (file) {
      ...
      showLoading();
      setStage(stageExtract, 'done');
      runAnalysis(text, 'Pasted Agreement');
    } else {
